$(document).ready(function () {
    ...
    $('#generate-report-btn').on('click', function () {
        // Grab the student name and class year from the report section
        const studentName = $('#student-name').text().trim(); // Get the student's name
        const classYear = $('#class-year').text().trim(); // Get the class/year
        const selectedTerm = $('#term-dropdown').val(); // Get the selected term
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // Make the AJAX call to generate the report
                $.ajax({
                    url: '/reports/generate_report/',  // Adjust URL if needed
                    type: 'POST',
                    data: JSON.stringify({
                        student_name: studentName,  // Send student's name and selected term
                    ...
                    success: function (response) {
                        if (response.success) {
                            Swal.fire({
                                ...
                                // Open the generated report HTML in a new tab with dynamic content
                                const reportWindow = window.open('', '_blank');
                                reportWindow.document.write('<html><head><title>Academic Report</title></head><body>');
                                reportWindow.document.write(response.report_html);  // Insert the HTML content here
